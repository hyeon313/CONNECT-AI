...
class MyApp(QMainWindow):
    ...
    def initUI(self):
        ...
        self.wg.predictionBtn.clicked.connect(self.predict_volume_image)
    ...
    def predict_currentImg(self):
        if self.is_opened:
            ...
            model.load_weights('C:/Users/jhLee/Desktop/final/model/47_0.0054.h5')
    ...
    def predict_volume_image(self):
        if self.is_opened:
            img = self.EntireImage
            img = np.expand_dims(img, axis=-1)
            print (img.shape)
            mask = np.zeros_like(self.EntireImage)
            ...
            for i in range(mask.shape[0]):
                prediction = model.predict(img[i:i+1])
                mask[i] =np.argmax(prediction, axis=-1)[0]
